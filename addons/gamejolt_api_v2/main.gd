...
func _ready():
	request_completed.connect(self._on_HTTPRequest_request_completed)
	pass
	
func auth_user(token, username):
	if busy: return
	busy = true
	var url = compose_url('auth/auth/authenticated', [username, token])
	username_cache = token
...
func compose_url(type, args):
	...
	for i in PARAMETERS[types[0]]:
		c += 1
		if !str(args[c]).is_empty() and str(args[c]) != '0':
			empty_counter += 1
			if empty_counter == 1:
				var parameter = i.replace('*', '?')
				final_url += parameter + str(args[c]).uri_encode()
			else:
				var parameter = i.replace('*', '&')
				final_url += parameter + str(args[c]).uri_encode()
	if empty_counter == 0:
	...
	s = s.md5_text()
	#print("GameJoltAPI: "+final_url + '&signature=' + s)
...
func _on_HTTPRequest_request_completed( result, response_code, headers, body ):
	...
	Gamejolt.result = json.get_data()
	emit_signal('api_' + request_type, json.get_data().response)
