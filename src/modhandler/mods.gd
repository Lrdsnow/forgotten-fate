...
func mod_scanner():
	...
	for file in files:
		if ".mod" in file:
			var json = JSON.new()
			var config = modsfolder + "/" + file + "/mod.config"
			
			if !FileAccess.file_exists(config):
				config = int_mods_folder + "/" + file + "/mod.config"
				if !FileAccess.file_exists(config):
					config = modsfolder + "/" + file + "/mod.json"
					if !FileAccess.file_exists(config):
						config = int_mods_folder + "/" + file + "/mod.json"
			...
			if FileAccess.file_exists(config):
				...
				if not outdated:
					...
					Global.mods.append(file)
					Global.unused_mods.append(file)
					Global.mod_datas[Global.lower(mod_data.mod)] = mod_data
					Global.mod_folders[Global.lower(mod_data.mod)] = file
...
func mod_pressed(button, autoload=false):
	...
	var mod_button = load("res://src/resources/ui/mod_button.tscn").instantiate()
	Global.used_mods.append(mod_name)
	Global.unused_mods.erase(mod_name)
	...
	if mod_data.has("full_game") and mod_data.full_game:
		...
		if !FileAccess.file_exists(mod_pck):
			mod_pck = Global.int_mods_folder + "/" + Global.mod_folders[Global.lower(mod_data.mod)] + "/" + mod_data.pck
